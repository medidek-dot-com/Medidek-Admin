...
function Addservices() {
  ...
  const DATASET_URL = 'https://swagstay-db-new.onrender.com/newservices';
  ...
  async function addDesignation(e) {
    if (designation.servicesName === '' || designation.status === '') {
  ...
  const getDesignation = async (departmentType_id) => {
    try {
      let response = await axios.get(
        `https://swagstay-db-new.onrender.com/newservices/${departmentType_id}`
      );
  ...
  async function updateDesignation(id) {
    ...
    try {
      let result = await axios.put(
        `https://swagstay-db-new.onrender.com/newservices/${id}`,
        designation
        // config
      );
      if (result.status === 200) {
        loadData();
        setDesignation({ status: '', servicesName: '' });
    ...
    <>
          ...
          >
            <div className='relative rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark'>
              <div className='flex flex-col gap-3 p-8'>
                <h1 className='text-xl font-semibold'>Edit Services Master</h1>
                <div>
                  ...
                  <input
                    ...
                    defaultValue={designation.servicesName}
                    onChange={(e) => {
                      setDesignation({
                        ...designation,
                  ...
                  <div>
                    <span className='text-meta-1'>
                      {designation.servicesName.length > 0 ? (
                        <></>
                      ) : (
              ...
              <button
                ...
                onClick={() => {
                  setIsOpen(false);
                  setDesignation({ servicesName: '', status: '' });
                }}
              >
                X
              </button>
      ...
      <DefaultLayout>
        <div className='flex items-center justify-between'>
          <h1 className='text-2xl font-bold'>Services Master</h1>
          <button
            onClick={() => {
              if (isOpen) {
                setIsOpen(false);
