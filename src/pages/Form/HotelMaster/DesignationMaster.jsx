...
function DesignationMaster() {
  ...
  const DATASET_URL = 'https://swagstay-db-new.onrender.com/designationmaster';
  ...
  async function addDesignation(e) {
    if (designation.designationname === '' || designation.status === '') {
  ...
  const getDesignation = async (departmentType_id) => {
    try {
      let response = await axios.get(
        `https://swagstay-db-new.onrender.com/designationmaster/${departmentType_id}`
      );
  ...
  async function updateDesignation(id) {
    ...
    try {
      let result = await axios.put(
        `https://swagstay-db-new.onrender.com/designationmaster/${id}`,
        designation
        // config
      );
      if (result.status === 200) {
        loadData();
        setIsOpen(false);
        setDesignation({ designationname: '', status: '' });
                ...
                <div>
                  ...
                  <div>
                    <span className='text-meta-1'>
                      {designation.designationname.length > 0 ? (
                        <></>
                      ) : (
